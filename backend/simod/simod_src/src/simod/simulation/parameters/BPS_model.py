...
from simod.data_attributes.types import CaseAttribute
from simod.data_attributes.types import EventAttribute
from simod.data_attributes.types import GlobalAttribute
...
class BPSModel:
    ...
    def to_prosimos_format(self) -> dict:
        ...
        if self.global_attributes is not None:
            attributes[GLOBAL_ATTRIBUTES_KEY] = [
                global_attribute.to_prosimos() for global_attribute in self.global_attributes
            ]
        if self.event_attributes is not None:
            attributes[EVENT_ATTRIBUTES_KEY] = [
                event_attribute.to_prosimos() for event_attribute in self.event_attributes
            ]
