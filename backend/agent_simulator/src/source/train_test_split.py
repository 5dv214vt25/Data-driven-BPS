...
def train_test_split(df, test_len=0.3, one_timestamp=False):
    log = df
    log = _sort_log(log)
    ...
    df_train = df_train.sort_values(by=["case_id", "pos_trace"], ascending=True)
    inc_traces = pd.DataFrame(df_train.groupby("case_id").last().reset_index())
...
def split_data(path_log, column_names, path_log_test=None):
    ...
    if path_log_test == None:
        ...
        df_train_big, df_test = train_test_split(df=df, test_len=0.2)
...
def load_data(path_or_buffer, file_extension, column_names):
    if file_extension == "csv" or file_extension == "gz":
    ...
    # print(df)
    df_train_big = df
    number_test_cases = len(set(df_train_big["case_id"]))

    return df_train_big, number_test_cases
