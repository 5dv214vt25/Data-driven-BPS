...
class ResourceAgent(Agent):
    ...
    def __init__(self, unique_id, model, resource, timer, deactivated, is_clone_of, contractor_agent=None):
        ...
        self.is_deactivated = deactivated  # For not including in the simulation
        self.is_clone_of = is_clone_of  # Int of "base" agent
        ...
        if self.resource in self.model.calendars.keys():
            ...
            self.calendar = next(
                (ids["calendar"] for role, ids in self.model.roles.items() if self.resource in ids["agents"]), None
            )
        self.timer = timer
        self.occupied_times = []
        # print(
        #     f"Created ResourceAgent in discovery! Id: {unique_id} Model: {self.model} Resource: {self.resource} Timer: {self.timer} Contractor_agent: {self.contractor_agent} Agent Type: {self.agent_type} Calendar: {self.calendar}"
        # )
    ...
    def step(self, last_possible_agent=False, parallel_activity=False, current_timestamp=None, perform_multitask=False):
        ...
        # picks the activity we'd run
        if not parallel_activity:
            idx = self.contractor_agent.new_activity_index
            activity = self.contractor_agent.activities[idx]
        else:
            idx = self.model.additional_activity_index
            activity = self.contractor_agent.case.additional_next_activities[idx]

        # figures out how long it will take
        duration = self.contractor_agent.get_activity_duration(self.resource, activity)

        # if start or end lie outside today’s window, bump time and bail
        if not self.start_time_in_calendar(current_timestamp) or not self.is_within_calendar(
            current_timestamp, duration
        ):
            # move case timestamp forward to the next calendar slot
            next_ts = self.set_time_to_next_availability_when_not_in_calendar(current_timestamp, duration)
            self.contractor_agent.case.current_timestamp = next_ts
            return

        if not self.is_deactivated:
            if not parallel_activity:
                self.contractor_agent.current_activity_index = idx
                self.perform_task(
                    current_timestamp,
                    duration,
                    activity,
                    last_possible_agent,
                    perform_multitask=perform_multitask,
                )
    ...
    def perform_task(
        ...
        activity,
        last_possible_agent,
        additional_act=False,
        additional_agent_counter=0,
        perform_multitask=False,
    ):
        sp = self.model.simulation_parameters or {}
        global_map = sp.get("activity_duration_map", {}) or {}
        agent_map_wrap = sp.get("agent_activity_duration_map", {}) or {}
        per_agent_map = agent_map_wrap.get(str(self.resource), {})

        if activity in per_agent_map:
            # 1) per-agent override
            activity_duration = per_agent_map[activity]
        elif activity in global_map:
            # 2) global override
            activity_duration = global_map[activity]

        if activity in self.timer:
            waiting_time = sample_from_distribution(distribution=self.timer[activity])
        else:
            waiting_time = 0
        ...
        if self.is_busy_until is not None and self.is_busy_until <= current_timestamp:
            self.is_busy = False

        if self.is_occupied(current_timestamp, activity_duration) == False or perform_multitask == True:
            ...
            if self.is_within_calendar(current_timestamp, activity_duration) or perform_multitask == True:
                ...
                if activity_duration != 0.0:
                    self.occupied_times.append(
                        (current_timestamp, current_timestamp + pd.Timedelta(seconds=activity_duration))
                    )
                ...
                self.contractor_agent.case.current_timestamp = current_timestamp + pd.Timedelta(
                    seconds=activity_duration
                )
                ...
                self.model.simulated_events.append(
                    {
                ...
                if last_possible_agent:  # then increase timer by x seconds to try to get an available agent later
                    ...
                    self.contractor_agent.case.current_timestamp = (
                        self.set_time_to_next_availability_when_not_in_calendar(current_timestamp, activity_duration)
                    )
    ...
    def set_current_time_to_next_available_slot(
        self,
    ):
        new_time_set = False
    ...
    def set_time_to_next_availability_when_not_in_calendar(self, current_timestamp, activity_duration):
        ...
        if working_hours is None:
            ...
            while next_working_day.strftime("%A").upper() not in [
                day_schedule["from"] for day_schedule in self.calendar
            ]:
                next_working_day += pd.Timedelta(days=1)

            for day_schedule in self.calendar:
                if (
                    day_schedule["from"] == next_working_day.strftime("%A").upper()
                    and day_schedule["to"] == next_working_day.strftime("%A").upper()
                ):
            ...
            try:
                next_possible_timestamp = datetime.combine(
                    next_working_day, datetime.strptime(next_working_hours[0], "%H:%M:%S").time()
                )
            except ValueError:
                next_possible_timestamp = datetime.combine(
                    next_working_day, datetime.strptime(next_working_hours[0], "%H:%M:%S%f").time()
                )
