...
class DurationDistribution:
    ...
    def generate_sample(self, size: int) -> list:
        ...
        if len(sample) < size:
            ...
            print(
                f"Warning when generating sample of distribution {self}. "
...
def get_best_fitting_distribution(
    ...
    :param user_input: Optional variable to choose distribution type
    ...
    # Here the user_input variable can be used to predetermine the distribution type.
    if user_input == "normal":
        distribution = DurationDistribution("norm", mean, var, std, d_min, d_max)
        return distribution
    if user_input == "mean":
        distribution = DurationDistribution("fix", mean, 0.0, 0.0, d_min, d_max)
        return distribution
    if user_input == "stddev":
        distribution = DurationDistribution("fix", std, 0.0, 0.0, d_min, d_max)
        return distribution
    # This clause is only for running pytest's.
    if user_input == "test":
        hardcoded_test_value = 2700  # 45*60 seconds
        distribution = DurationDistribution("fix", hardcoded_test_value, 0.0, 0.0, d_min, d_max)
        return distribution
    # If no distribution type is chosen one is determined by finding the best fitting one.
    if user_input is None:
        ...
        raise ValueError("Unknown user input for distribution type")
